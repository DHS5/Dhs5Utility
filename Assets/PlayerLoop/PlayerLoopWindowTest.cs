...
public class PlayerLoopWindowTest : EditorWindow
{
    ...
    private void RemoveSystem(PlayerLoopSystem target, PlayerLoopSystem parentSystem)
    {
        ...
        {
            var system = parentSystem.subSystemList[i];
            if (system.type == target.type)
            {
                // create a list of the subsystems, its easier to work with 
                var newList = new List<PlayerLoopSystem>(parentSystem.subSystemList);

                // remove the target
                newList.RemoveAt(i);
                parentSystem.subSystemList = newList.ToArray();

                nextPlayerLoop = currentPlayerLoop; // copy the current loop...
                // and plug in our updated parent into it.
                for (int j = 0; j < nextPlayerLoop.subSystemList.Length; j++)
                {
                    if (nextPlayerLoop.subSystemList[j].type == parentSystem.type)
                    {
                        nextPlayerLoop.subSystemList[j] = parentSystem;
                    }
                }
